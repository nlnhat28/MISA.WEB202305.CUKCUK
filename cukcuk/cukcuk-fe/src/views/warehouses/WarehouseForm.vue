                ...
                <m-form-row>
                    ...
                    >
                        <m-textfield
                            ...
                            :validate="validateWarehouseCode"
                            :label="fields.warehouseCode.label"
                            :maxLength="fields.warehouseCode.maxLength"
                            ref="WarehouseCode"
                        >
                        </m-textfield>
                    </m-form-item>
                ...
                <m-form-row>
                    ...
                    >
                        <m-textfield
                            ...
                            :validate="validateWarehouseName"
                            :label="fields.warehouseName.label"
                            :maxLength="fields.warehouseName.maxLength"
                            ref="WarehouseName"
                        >
                        </m-textfield>
                    </m-form-item>
...
export default {
    ...
    mounted() {
        this.refs = [
            this.$refs.Address,
            this.$refs.WarehouseName,
            this.$refs.WarehouseCode,
        ];
    ...
    computed: {
        ...
        formTitle() {
            switch (this.mode) {
                case this.$enums.formMode.create:
                    return this.$resources["vn"].createWarehouse;
                case this.$enums.formMode.update:
                    return this.$resources["vn"].updateWarehouse;
                case this.$enums.formMode.duplicate:
                    return this.$resources["vn"].duplicateWarehouse;
                default:
                    return this.$resources["vn"].createWarehouse;
            }
        },
        /**
         * Message of warehouse on toast
         *
         * Author: nlnhat (02/07/2023)
         * @return {*} New title update or create
         */
        messageOnToast() {
            ...
            const content = `${title} ${this.$resources["vn"].warehouse} <${this.warehouse.WarehouseName}>`;
            return content;
        },
        /**
         * Map warehouse store
    ...
    methods: {
        ...
        async createWarehouse() {
            try {
                const response = await warehouseService.post(this.warehouse);
                if (response?.status == this.$enums.status.created) {
                    this.warehouse.WarehouseId = response.data;
        ...
        async onClickSave() {
            try {
                if (this.isValidForm) {
                    await this.onSave();
                    if (this.isSuccessResponseFlag == true) {
                        this.$emit("emitUpdateWarehouse", this.warehouse.WarehouseId);
         ...
         */
        onClickCloseForm() {
            if (!this.sameObject(this.originalWarehouse, this.warehouse))
